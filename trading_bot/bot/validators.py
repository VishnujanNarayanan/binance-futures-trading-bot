...
def validate_inputs(symbol: str, side: str, order_type: str, quantity: float, price: Optional[float] = None, stop_price: Optional[float] = None):
    ...
    if type_upper in ["LIMIT", "STOP_LIMIT"] and (price is None or price <= 0):
        raise ValueError(f"Price is required and must be greater than 0 for {type_upper} orders.")
            
    if type_upper == "STOP_LIMIT" and (stop_price is None or stop_price <= 0):
